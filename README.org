...
Candidates longer than ~nucleo-completion-long-candidate-threshold~ characters
are filtered but not scored by the Rust module.  They are appended after scored
matches in their original order, which keeps unusually long lines from
dominating scoring time while preserving them as selectable matches.  Set the
option to nil to score every candidate regardless of length.
