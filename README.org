...
still-running filter pass.

~nucleo-completion-use-cache~ is enabled by default.  During one completion
session, when the current query extends the previous query in the same
~table~/~pred~/prefix context, ~nucleo-completion~ reuses the previously
narrowed candidates instead of asking the original completion table for the full
candidate set again.

The cache is intentionally conservative and is skipped when
~nucleo-completion-regexp-functions~ or an existing ~completion-regexp-list~ is
active, since custom regexp expanders may not be monotonic as the query grows.

Tie sorting with ~nucleo-completion-sort-ties-by-length~ and
~nucleo-completion-sort-ties-alphabetically~ is handled by the Rust module when
the bundled module is new enough.  Older modules fall back to Emacs Lisp sorting
for compatibility.
...
If Migemo regexp expansion is slow in large completion sets, cache the generated
regexp for each term. The following example also caches nil results for invalid
regexps and clears the cache after it grows past 1024 entries:
...
Complete ~use-package~ example with cached Migemo and Corfu:
...
regexp expander are not appended after all fuzzy matches.  They receive a
lightweight synthetic score from the regexp match position, match length, and
candidate length, then join the same score ordering as Nucleo matches.  This
keeps Migemo-style dictionary matches near similarly strong literal/fuzzy
matches while avoiding a second regexp pass during sorting.

Synthetic regexp scores are only used for ordering.  Candidates that match only
through a regexp expander are not dimmed by score-band highlighting; only their
