...
Older modules without the batch sorting API are ignored and the package uses the
simple Emacs Lisp fallback filter instead.  The fallback only narrows
candidates; it preserves input order and does not compute scores, apply tie
sorting, or add score-band highlighting.
